class rectangleDrawer {
  ...
  isCorner(div, currPos) {
    ...
    const nearTopLeftCorner = this.shouldCalculateTriangleAreas(
      currPos,
      domRectangle,
      0
    );
    const nearTopRightCorner = this.shouldCalculateTriangleAreas(
      currPos,
      domRectangle,
      1
    );
    const nearBottomRightCorner = this.shouldCalculateTriangleAreas(
      currPos,
      domRectangle,
      2
    );
    const nearBottomLeftCorner = this.shouldCalculateTriangleAreas(
      currPos,
      domRectangle,
      3
    );
  ...
  isBorderGrab(div, currPos) {
    ...
    if (
      currPos.y >= domRectangle.top - sensitivity &&
      currPos.y <= domRectangle.top + sensitivity &&
      currPos.x > domRectangle.left + sensitivity &&
      currPos.x < domRectangle.right - sensitivity
    ) {
      return 0; // Top
    }
    ...
    // Right border (excluding top and bottom sensitivity areas)
      ...
      currPos.x <= domRectangle.right + sensitivity &&
      currPos.y > domRectangle.top + sensitivity &&
      currPos.y < domRectangle.bottom - sensitivity
    ) {
      return 1;
    }
  ...
  isWithinRectangle(currPos, topLeftCoord, bottomRightCoord) {
    const validX =
      currPos.x >= topLeftCoord.x && currPos.x <= bottomRightCoord.x;
    const validY =
      currPos.y >= topLeftCoord.y && currPos.y <= bottomRightCoord.y;
  ...
  shouldCalculateTriangleAreas(currPos, domRectangle, cornerType) {
    ...
    let topLeftProjection = this.coord(
      point.x - this.sensitivity,
      point.y - this.sensitivity
    );
    let bottomRightProjection = this.coord(
      point.x + this.sensitivity,
      point.y + this.sensitivity
    );

    return this.isWithinRectangle(
      currPos,
      topLeftProjection,
...
mode -1 means do nothing when mouse moves
  ...
  createModeSizeChange(currPos) {
    ...
    if (currPos.x <= this.createdNodeStartPos.x)
      this.currNode.style.left = currPos.x + 'px';

    if (currPos.y <= this.createdNodeStartPos.y)
      this.currNode.style.top = currPos.y + 'px';
  }
  ...
  handleMouseMove(event) {
    ...
    else if (this.operatingMode == 1) this.handleBorderGrab(currPos);
    //resize mode
    else if (this.operatingMode == 2) this.handleCornerGrab(currPos);

    if (this.operatingMode != -1) this.setDimensionBoxDimensions(this.currNode);
  }
  ...
  setDimensionBoxDimensions(div) {
    const domRectangle = div.getBoundingClientRect();
    //integers only
    const text = Math.round(domRectangle.width) + ' x ' + 
    Math.round(domRectangle.height);
